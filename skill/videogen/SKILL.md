...
- **Bilateral scenes (two armies/flag sets): generate the background as ONE image** with a left/right spatial prompt ("on the LEFT side ... banners; on the RIGHT side ... flags; empty corridor between") — unified lighting/perspective — plus a spatial prompt lock in the video prompt and a fixed-camera instruction ("single continuous static wide shot, no split screen, no panning between sides"). Do NOT stitch two half-backgrounds together: even with a blurred seam, the color-temperature split leaks into the video as an artificial split-screen feel. Solo shots (drop LiconMSR input "2", single ref into slot 1) remain useful for close-ups.
