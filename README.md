...
- **MSR multi-subject** (`workflows/ltx23-msr-multisubject.json`) — feed up to 4 character reference images + a background reference DIRECTLY into the LiconMSR node (requires the community `LTX-2.3-Licon-MSR-V1` LoRA): both/all likenesses hold simultaneously in one continuous shot, ~0.56s/frame at 50fps. Start your prompt with "Maintain strong reference consistency for both characters and the background — do not change faces, hairstyles, costumes, or the background layout." Best route for multi-character dialogue scenes. **Brand or multi-character videos: prefer ONE continuous MSR shot** — feed each real person photo as a character ref plus a real location photo as the background ref, and let MSR render everyone natively into the scene (lighting integrates for free, no compositing/relighting, no cuts). Beats intercutting separate lip-sync clips, which reintroduce a pasted-in look and per-cut inconsistency. Tips: the background reference DOMINATES — any character visible in it will be generated, so inspect bg images before feeding; for scenes with two opposing sides (armies, flag sets), generate the background as ONE image with a left/right spatial prompt (each side's elements on its own side, empty ground between; unified lighting) plus a spatial lock and a fixed-camera line in the video prompt — MSR keeps the sides separate in a single continuous two-shot; avoid stitching two half-backgrounds (the seam/color split leaks into the video as a split-screen feel) (per-side solo shots via dropping LiconMSR input `"2"` also work for close-ups); need an exact glyph or logo on a banner? draw it onto the background reference (e.g. PIL) — MSR reproduces it faithfully.
